...
interface AboutClientProps {
    ...
    realmsData?: {
        realms?: Array<{ title: string; icon: string; description: string }>;
    };
}

export default function AboutClient({ sanityData, statsData, realmsData }: AboutClientProps) {
    const aboutImages = [
        '/images/aboutme1.jpg',
        '/images/aboutme2.jpg',
        '/images/aboutme3.jpg',
    ...
    // Default static text if sanityData is missing
    const defaultBio = [
        "As a passionate educator and researcher at [ESTIN], I bridge the gap between theoretical data science and practical software engineering. I am a multifaceted technology professional with a passion for innovation, currently pursuing my [PhD in AI & Data Science] while architecting scalable full-stack solutions.",
        "My journey spans across developing sophisticated applications, conducting [groundbreaking research] in machine learning, and training [hundreds of professionals] worldwide. I believe in the power of technology to transform lives and am committed to making complex concepts accessible to all.",
        "Whether it's architecting [scalable systems], mentoring students, or leading workshops, I approach every challenge with [curiosity], [dedication], and a drive for [excellence]."
    ];

    // Split text by double newline to handle paragraphs
    const bioParagraphs = sanityData?.About_text
        ? sanityData.About_text.split(/\n\s*\n/)
        : defaultBio;
                ...
                <div className="space-y-12">
                    ...
                    >
                        {(realmsData?.realms && realmsData.realms.length > 0) ? (
                            realmsData.realms.map((realm, index) => {
                                const IconComponent = IconMap[realm.icon] || Brain;
                                return (
                                    <motion.div key={index} variants={itemVariants}>
                                        <RealmCard
                                            icon={<IconComponent size={48} className="text-indigo-400" />}
                                            title={realm.title}
